...
int main(int argc, char * argv[]) {
    ...
    for (int i = 1; i <= NB_CLIENTS; i++) {
        ...
        if ( (nbOctetsRecus = recvfrom(sock_serveur, buf, sizeof(buf), 0,(struct sockaddr *) &sockaddr_client, &size)) == -1)exitErreur("recvfrom");



    cout << string(buf,nbOctetsRecus) << endl;

        if (sendto(sock_serveur, buf, nbOctetsRecus, 0,(struct sockaddr *) &sockaddr_client, sizeof(sockaddr_client))== -1)
            exitErreur("sendto");
